"""Command line interface for quant_pipeline."""
...
def ingest_ohlcv_cmd(exchange, symbols, timeframe, since, until, out) -> None:
    """Download and store OHLCV data from CCXT exchanges."""

    sym_list = [s.strip() for s in symbols.split(",") if s.strip()]
    since_ms = int(pd.Timestamp(since, tz="UTC").value // 1_000_000) if since else None
    until_ms = int(pd.Timestamp(until, tz="UTC").value // 1_000_000) if until else None
    ingest_ohlcv_ccxt(
        exchange,
        sym_list,
        timeframe=timeframe,
